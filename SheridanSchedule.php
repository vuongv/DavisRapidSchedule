...
<html>
    ...
    <body>
        ...
        <div>
            ...
            //connect to database
                try{
                    $dbh = new PDO("mysql:host=localhost;dbname=vuongv_SHERIDAN","vuongv","Q3sLpnT@3E!&M");
                    }catch (Exception $ex){
                ...
                if(isset($_POST['showMondayDAV'])) { 
                        $command = "select Facility_ID, max(mtg_start) as start_time, max(mtg_end) as end_time,Room_Description, IT from `DAV_MONDAY`
                        ...
                        while($row = $stmt->fetch()){
                            ...
                            echo "<p><button onclick=\"sortTable(1)\">sort by Room</button><button onclick=\"sortTable(2)\">sort by Start Time</button><button onclick=\"sortTable(3)\">sort by End Time</button><button onclick=\"sortTable(4)\">sort by Room Description</button></p>";
                            echo "</table>";
                }
                //DAV_TUESDAY PHP
                ...
                //HMC_SATURDAY PHP
                if(isset($_POST['showSaturdayHMC'])) { 
                        $command = "select Facility_ID, max(mtg_start) as start_time, max(mtg_end) as end_time,Room_Description, IT from `HMC_SATURDAY`
                                    group by Facility_ID order by end_time ASC;";
                        $stmt = $dbh->prepare($command);
                        $stmt->execute();
                        echo "<h1>HMC Campus Friday</h1>";
                ...
                //HMC_SUNDAY PHP
                if(isset($_POST['showSundayHMC'])) { 
                        $command = "select Facility_ID, max(mtg_start) as start_time, max(mtg_end) as end_time,Room_Description, IT from `HMC_SUNDAY`
                                    group by Facility_ID order by end_time ASC;";
                        $stmt = $dbh->prepare($command);
                        $stmt->execute();
                        echo "<h1>HMC Campus Friday</h1>";
